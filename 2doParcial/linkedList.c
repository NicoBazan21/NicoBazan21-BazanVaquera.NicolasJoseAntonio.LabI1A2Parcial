   ...
   // Node* auxNode = NULL;
    ...
    if(this != NULL && nodeIndex>-1 && nodeIndex < ll_len(this))
    {
        ...
        {*/
            ...
            //auxNode = this->pFirstNode;//direccion del nodo 0
            pNode=this->pFirstNode;
            ...
            while(nodeIndex > 0)
            {
                ...
                nodeIndex--;
            } //nota: ESTO PUEDE IR SOLO EN EL CODIGO SIN IF Y ELSE (sin mayus jaja)
...
/** \brief Permite obtener el pElement
 *
 * \param this LinkedList* Puntero a la lista
 * \param nodeIndex int Ubicacion del elemento a obtener
 * \return void* Retorna    (NULL) Error: si el puntero a la lista es NULL o (si el indice es menor a 0 o mayor al len de la lista)
                            (pElement) Si funciono correctamente
                            *lo que esta guardado en el campo pelement de ese vagon*
 *
 */
...
{
    int returnAux = -1;
    Node* noteToRemove = NULL;
    Node* previousNode = NULL;

    if(this != NULL && index >= 0 && index < ll_len(this))
    {
        noteToRemove = getNode(this, index);

        if(noteToRemove != NULL)
        {
            if(index==0)
            {
                this->pFirstNode= noteToRemove->pNextNode;
            }
            else
            {
                previousNode=getNode(this, index-1);
                previousNode->pNextNode=noteToRemove->pNextNode;
            }
            free(noteToRemove);
            this->size--;
            returnAux=0;
...
int ll_sort(LinkedList* this, int (*pFunc)(void* ,void*), int order)
{
    ...
    if(this != NULL && pFunc != NULL && order >= 00 && order <=1)
    {
        int tam = ll_len(this);
